...
void OnlineSupTaskDrivDicLeaDecFusJointQuadC(const DataMat& XArr,
  ...
  DataMat& DSup, std::vector<DataMat>& W, DataMat& b, int rank, int ProcSize) {
  ...
  for (int iteration = 0; iteration < iter; ++iteration) {
    for (int t = 0; t < (N - N % batchSize); t += batchSize) {
      ...
      **********************************************/
      ////////////////////////////////////////////
      DataMat XArr_sample, tmp_alpha;
      if (rank == root) {
        XArr_sample = colm(XArr_randperm, range(t, batchSize + t - 1));
        tmp_alpha = zeros_matrix<Dtype>(d * S, batchSize + t - 1);
      }
      ...
      MPI_Gather(Alpha0.begin(),Alpha0.nr()*batchSize/ProcSize,MPI_DOUBLE,tmp_alpha.begin(),S*d*batchSize/ProcSize,MPI_DOUBLE,root,MPI_COMM_WORLD);
        
      if (rank == root) {
        for (int j = 0; j < batchSize; ++j) {
          DataMat YArr_sample = colm(Y_randperm, j + t);
          set_colm(Atemp, j) = colm(tmp_alpha, j);
          GetBetaTemp2(colm(tmp_alpha, j), YArr_sample, W, b, DoTDoAll, d, S, j, lambda,
            Atemp2, BetaTemp2);
        }//for j
